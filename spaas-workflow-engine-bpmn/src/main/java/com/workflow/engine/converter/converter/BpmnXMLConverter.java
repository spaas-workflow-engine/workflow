...
import com.workflow.engine.converter.converter.alfresco.AlfrescoStartEventXMLConverter;
import com.workflow.engine.converter.converter.alfresco.AlfrescoUserTaskXMLConverter;
import com.workflow.engine.converter.converter.child.DocumentationParser;
import com.workflow.engine.converter.converter.child.IOSpecificationParser;
import com.workflow.engine.converter.converter.child.MultiInstanceParser;
import com.workflow.engine.converter.converter.export.*;
import com.workflow.engine.converter.converter.parser.*;
import com.workflow.engine.converter.converter.util.BpmnXMLUtil;
import com.workflow.engine.converter.exceptions.XMLException;
import com.workflow.engine.io.InputStreamProvider;
...
public class BpmnXMLConverter implements BpmnXMLConstants {
    ...
    protected FlowNode getFlowNodeFromScope(String elementId, BaseElement scope) {
        ...
        if (StrUtil.isNotEmpty(elementId)) {
    ...
    public byte[] convertToXML(BpmnModel model, String encoding) {
        try {
            ...
            for (com.workflow.engine.model.Process process : model.getProcesses()) {
    ...
    protected void createXML(FlowElement flowElement, BpmnModel model, XMLStreamWriter xtw) throws Exception {
        ...
        if (flowElement instanceof SubProcess) {
            ...
            if (StrUtil.isNotEmpty(subProcess.getName())) {
            ...
            if (subProcess instanceof EventSubProcess) {
                ...
                if (StrUtil.isNotEmpty(adhocSubProcess.getOrdering())) {
            ...
            if (StrUtil.isNotEmpty(subProcess.getDocumentation())) {
            ...
            if (subProcess instanceof AdhocSubProcess) {
                AdhocSubProcess adhocSubProcess = (AdhocSubProcess) subProcess;
                if (StrUtil.isNotEmpty(adhocSubProcess.getCompletionCondition())) {
