 ...
 */
package com.workflow.engine.converter.converter.util;

import cn.hutool.core.util.StrUtil;
import com.workflow.engine.converter.constants.BpmnXMLConstants;
import com.workflow.engine.converter.converter.child.*;
...
public class BpmnXMLUtil implements BpmnXMLConstants {
    ...
    public static ExtensionElement parseExtensionElement(XMLStreamReader xtr) throws Exception {
        ...
        for (int i = 0; i < xtr.getAttributeCount(); i++) {
            ...
            if (StrUtil.isNotEmpty(xtr.getAttributeNamespace(i))) {
                extensionAttribute.setNamespace(xtr.getAttributeNamespace(i));
            }
            if (StrUtil.isNotEmpty(xtr.getAttributePrefix(i))) {
    ...
    public static void writeDefaultAttribute(String attributeName, String value, XMLStreamWriter xtw) throws Exception {
        if (StrUtil.isNotEmpty(value) && !"null".equalsIgnoreCase(value)) {
    ...
    protected static void writeExtensionElement(ExtensionElement extensionElement, Map<String, String> namespaceMap, XMLStreamWriter xtw) throws Exception {
        if (StrUtil.isNotEmpty(extensionElement.getName())) {
            Map<String, String> localNamespaceMap = new HashMap<>();
            if (StrUtil.isNotEmpty(extensionElement.getNamespace())) {
                if (StrUtil.isNotEmpty(extensionElement.getNamespacePrefix())) {
            ...
            for (List<ExtensionAttribute> attributes : extensionElement.getAttributes().values()) {
                for (ExtensionAttribute attribute : attributes) {
                    if (StrUtil.isNotEmpty(attribute.getName()) && attribute.getValue() != null) {
                        if (StrUtil.isNotEmpty(attribute.getNamespace())) {
                            if (StrUtil.isNotEmpty(attribute.getNamespacePrefix())) {
